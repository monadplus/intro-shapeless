...
object DerivingLabelled extends App {

  import shapeless.labelled.FieldType
  import shapeless.{::, HList, HNil, Lazy, Witness}

  import Json._
  import Json.JsEncoder._

  case class IceCream(flavour: String, numCherries: Int, inCone: Boolean)
...
//  ))
  ...
  scala.Console.println {
    s"Ice cream: ${JsEncoder[IceCream].encode(iceCream)}"
  }

  scala.Console.println {
    s"Store: ${JsEncoder[Store].encode(store)}"
  }
  ...
  scala.Console.println {
    s"Shape: ${JsEncoder[Shape].encode(shape)}"
  }
}
